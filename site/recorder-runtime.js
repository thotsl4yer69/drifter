(()=>{'use strict';const $=s=>document.querySelector(s),$$=s=>[...document.querySelectorAll(s)];const reduce=matchMedia('(prefers-reduced-motion: reduce)');
const canvas = $('#ribbon'), scene = $('#scene'), range = $('#time-range');
  ...
  if($('#jump-event'))$('#jump-event').disabled=false;
