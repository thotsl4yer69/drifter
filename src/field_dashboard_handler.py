...
These routes are deliberately local/hotspot-only and provide touch-safe,
structured operations for vehicle-link setup and RF field work. The existing
DashboardHandler remains the fallback for every legacy route.
"""
...
def _json_from_stdout(result: subprocess.CompletedProcess) -> dict:
    ...
    try:
        ...
        pass
    return {"ok": True, "rc": result.returncode, "output": result.stdout.strip()[:4000]}
...
class FieldDashboardHandler(DashboardHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/field/obd/status":
            result = _run(["/usr/local/bin/drifter", "obd", "--json", "status"], timeout=8)
            return self._field_json(_json_from_stdout(result), 200 if result.returncode == 0 else 503)
    ...
    def do_POST(self) -> None:
        ...
        if path == "/api/field/obd/scan":
            ...
            result = _run(["/usr/local/bin/drifter", "obd", "--json", "scan", "--seconds", str(seconds)], timeout=seconds + 8)
            return self._field_json(_json_from_stdout(result), 200 if result.returncode == 0 else 503)
        ...
        if path == "/api/field/obd/auto":
            result = _run(["sudo", "-n", "/usr/local/bin/drifter", "obd", "setup", "--seconds", "7"], timeout=35)
            return self._field_json({
        ...
        if path == "/api/field/obd/select":
            ...
            if mode == "bluetooth":
                ...
                try: port = int(body.get("port", 35000))
                ...
                try: baud = int(body.get("baud", 38400))
                except (TypeError, ValueError): baud = 0
        ...
        if path == "/api/field/obd/pair":
            ...
            try:
                result = subprocess.run(
                    ...
                    input=script, capture_output=True, text=True, timeout=25, check=False,
                )
            except (OSError, subprocess.SubprocessError) as exc:
                return self._field_json({"ok": False, "error": str(exc)}, 503)
            text = (result.stdout + "\n" + result.stderr).strip()
            paired = "Pairing successful" in text or "Paired: yes" in text or "already paired" in text.lower()
            return self._field_json({"ok": paired, "rc": result.returncode, "output": text[-5000:]})
        ...
        if path == "/api/field/obd/wifi-connect":
            ...
            result = _run(argv, timeout=25)
            return self._field_json({"ok": result.returncode == 0, "rc": result.returncode,
                                     "output": result.stdout.strip()[-2000:], "error": result.stderr.strip()[-1000:]})
        ...
        if path == "/api/field/rf/command":
            ...
            if action in {"hunt_start", "capture", "zoom", "listen"}:
                try: freq = float(body.get("freq_mhz"))
                except (TypeError, ValueError): return self._field_json({"ok": False, "error": "freq_mhz required"}, 400)
                if not 24 <= freq <= 1766:
                    return self._field_json({"ok": False, "error": "freq_mhz outside RTL-SDR range"}, 400)
                payload["freq_mhz"] = freq
            if action == "capture": payload["duration_s"] = max(1, min(30, float(body.get("duration_s", 15))))
            if action == "zoom": payload["span_khz"] = max(50, min(5000, int(body.get("span_khz", 500))))
            if action == "listen":
        ...
        if path == "/api/field/display/recover":
            result = _run(["sudo", "-n", "/usr/local/bin/drifter", "display", "recover"], timeout=20)
            return self._field_json({"ok": result.returncode == 0, "rc": result.returncode,
                                     "output": result.stdout.strip()[-3000:], "error": result.stderr.strip()[-1000:]})
